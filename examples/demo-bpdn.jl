...
function run_bpdn_solvers(compound=1)
  ...
  # EXACT
  context_exact = ProxTVContext(model.meta.nvar, :lp, p,
    κs=1e-7, λ=λ, dualGap=eps(), use_absolute_criterion=true)
  hp_exact = NormLp(λ, p, context_exact)
  reset!(f)
  out_exact = iR2N(f, hp_exact, options, x0=f.meta.x0)

  # INEXACT
  context_inexact = ProxTVContext(model.meta.nvar, :lp, p,
    κs=1e-7, λ=λ, dualGap=eps())
...
plot_bpdn_compare(out_exact, out_inexact, sol; name="bpdn-exact-vs-inexact")