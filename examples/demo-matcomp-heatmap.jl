...
function plot_projection_experiment(; λ=1e-1, p=1.1, kwargs...)
  ...
  # échelle commune pour plots 1–2
  maxR12 = maximum((maximum(R1), maximum(R2)))
  # échelle propre pour plot 3
  maxR3 = maximum(R3)

  fig, ax = subplots(1, 3, figsize=(13, 3), constrained_layout=true)

  im1 = ax[1].imshow(R1, cmap="magma", vmin=0.0, vmax=maxR12)
  ax[1].axis("off")
  ax[1].set_title(L"$|X_{\mathrm{inexact}} - A| \,\, (\kappa_s = 10^{-7})$", fontsize=18)

  im2 = ax[2].imshow(R2, cmap="magma", vmin=0.0, vmax=maxR12)
  ax[2].axis("off")
  ax[2].set_title(L"$|X_{\mathrm{exact}} - A|$", fontsize=18)

  fig.colorbar(im1, ax=ax[1:2], shrink=0.75)

  im3 = ax[3].imshow(R3, cmap="viridis", vmin=0.0, vmax=maxR3)
  ax[3].axis("off")
  ax[3].set_title(L"$|X_{\mathrm{inexact}} - X_{\mathrm{exact}}|$", fontsize=18)

  fig.colorbar(im3, ax=ax[3], shrink=0.75)

  savefig("demo-matcomp-comparison.pdf")
  println("Saved figure to demo-matcomp-comparison.pdf")
end
