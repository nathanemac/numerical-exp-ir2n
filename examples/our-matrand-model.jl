...
function MatrixProjection(m::Int, n::Int, ω::AbstractVector{<:Integer})
  mask = falses(m * n)
  mask[ω] .= true
  MatrixProjection(mask, m, n)
end
...
function matrix_completion_model(xs, B, ω)
  ...
  projection = MatrixProjection(m, n, ω)
...
"""
    model, nls_model, sol, projection = random_matrix_completion_model(; kwargs...)
...
* `nls_model`: `FirstOrderNLSModel`
* `sol`: exact solution `vec(A)`
* `projection`: a `MatrixProjection` encoding the operator `P`, providing direct access to the sampling mask for downstream comparisons such as `PX`, `PA`, or `P(X - A)`.
"""
...
"""
    model, nls_model, sol, projection = MIT_matrix_completion_model()

A special case of matrix completion problem in which the exact image is a noisy
MIT logo. The returned `projection` is the sampling operator matching the MIT mask.
